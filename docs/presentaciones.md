---
sidebar_position: 0
title: Página de Presentaciones
description: Presentaciones interactivas de cada tema
---

# 🎬 Presentaciones

Presentaciones interactivas en formato Reveal.js para cada tema de la unidad.

## Temas disponibles

| Tema | Descripción | Enlace |
|------|-------------|--------|
| **Tema 1** | Introducción a la Electrónica Digital | [Ver presentación](/presentacion/tema1.html) |
| **Tema 2** | Sistemas de Numeración | [Ver presentación](/presentacion/tema2.html) |
| **Tema 3** | Álgebra de Boole | [Ver presentación](/presentacion/tema3.html) |
| **Tema 4** | Puertas Lógicas | [Ver presentación](/presentacion/tema4.html) |
| **Tema 5** | Análisis y Diseño de Circuitos | [Ver presentación](/presentacion/tema5.html) |
| **Tema 6** | Circuitos Combinacionales | [Ver presentación](/presentacion/tema6.html) |
| **Tema 7** | Prácticas de Taller | [Ver presentación](/presentacion/tema7.html) |

---

## 📋 Instrucciones de uso

### Navegación
- **Flechas del teclado**: Siguiente/Anterior diapositiva
- **Espacio**: Avanzar
- **Esc**: Vista general de diapositivas
- **F**: Pantalla completa
- **S**: Vista del presentador (con notas)

### Para el profesorado
Las presentaciones incluyen notas del orador accesibles pulsando **S**. 
Cada presentación está optimizada para proyección en aula.

---

## 🔧 Recursos adicionales

Durante las presentaciones puedes acceder a:
- [Simulador Tinkercad](https://www.tinkercad.com/circuits) para demostraciones en vivo
- [Falstad Circuit Simulator](https://falstad.com/circuit/) para circuitos más complejos
